...
main(){
  ...
  db.open().then((c){
    ...
    return coll.find(where.gt("my_field", 995).sortBy('my_field')).forEach((v)=>print(v));
  }).then((val){   
    ...
    return coll.findOne(where.eq("my_field", 17).fields(['str_field']));
      ...
      return coll.find(where.gt("my_field", 995).or(where.lt("my_field", 10)).and(where.match('str_field', '99'))).forEach((v)=>print(v));
