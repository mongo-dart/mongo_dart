...
class ConnectionPool {
  ...
  ConnectionPool(this.concurrency, this.dbFactory) {
    _pool = new Pool(concurrency);
  }

  /// Connects to the database, using an existent connection, only creating a new one if
  /// the number of active connections is less than [concurrency].
  Future<Db> connect() {
    return _pool.withResource<Db>(() async {
      int i = _index;
      if (_index >= concurrency) _index = 0;
